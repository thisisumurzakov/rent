...
from ..serializer import TruckSerializer, TruckAddSerializer
from main.serializers.product_serializer import ProductAddSerializer


class TruckGetView(APIView):
    permission_classes = (AllowAny,)

    def get(self, request, *args, **kwargs):
        obj = get_object_or_404(Truck.objects.select_related("product"),
                                product__slug=kwargs["slug"], product__draft=False)
        data = TruckSerializer(obj).data
...
class TruckView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        serializer = TruckAddSerializer(data=request.data)
    ...
    def patch(self, request, *args, **kwargs):
        ...
        if request.data:
            obj = get_object_or_404(Truck.objects.select_related('product'), product__slug=kwargs['slug'], product__author=request.user)
            s = TruckAddSerializer(obj, data=request.data, partial=True)
            s.is_valid(raise_exception=True)
            s.save()
