...
from ..models import Other_t
from ..serializer import Other_tSerializer, Other_tAddSerializer
from main.serializers.product_serializer import ProductAddSerializer


class Other_tGetView(APIView):
    permission_classes = (AllowAny,)

    def get(self, request, *args, **kwargs):
        obj = get_object_or_404(Other_t.objects.select_related("product"),
                                product__slug=kwargs["slug"], product__draft=False)
        data = Other_tSerializer(obj).data
...
class Other_tView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        serializer = Other_tAddSerializer(data=request.data)
    ...
    def patch(self, request, *args, **kwargs):
        ...
        if request.data:
            obj = get_object_or_404(Other_t.objects.select_related('product'), product__slug=kwargs['slug'], product__author=request.user)
            s = Other_tAddSerializer(obj, data=request.data, partial=True)
            s.is_valid(raise_exception=True)
            s.save()
