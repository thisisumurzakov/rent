...
from ..models import Other_r
from ..serializer import Other_rSerializer, Other_rAddSerializer
from main.serializers.product_serializer import ProductAddSerializer


class Other_rGetView(APIView):
    permission_classes = (AllowAny,)

    def get(self, request, *args, **kwargs):
        obj = get_object_or_404(Other_r.objects.select_related("product"),
                                product__slug=kwargs["slug"], product__draft=False)
        data = Other_rSerializer(obj).data
...
class Other_rView(APIView):
    ...
    def patch(self, request, *args, **kwargs):
        ...
        if request.data:
            obj = get_object_or_404(Other_r.objects.select_related('product'), product__slug=kwargs['slug'], product__author=request.user)
            s = Other_rAddSerializer(obj, data=request.data, partial=True)
            s.is_valid(raise_exception=True)
            s.save()
