...
from ..serializer import OfficeSerializer, OfficeAddSerializer
from main.serializers.product_serializer import ProductAddSerializer


class OfficeGetView(APIView):
    permission_classes = (AllowAny,)

    def get(self, request, *args, **kwargs):
        obj = get_object_or_404(Office.objects.select_related("product"),
...
class OfficeView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        with transaction.atomic():
            try:
                ...
                serializer = OfficeAddSerializer(data=request.data)
    ...
    def patch(self, request, *args, **kwargs):
        ...
        if request.data:
            obj = get_object_or_404(Office.objects.select_related('product'), product__slug=kwargs['slug'], product__author=request.user)
            s = OfficeAddSerializer(obj, data=request.data, partial=True)
            s.is_valid(raise_exception=True)
            s.save()
